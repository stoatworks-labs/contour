...
terrain card and reports any that made no difference at all.

    python3 tools/sweep.py [--binary build/cntest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The sea's audio controls need audio.** With no spectrum every bin is 0,
the drive is 0, and Audio Rise, Audio Mode and Audio Band all provably do
nothing. `cntest --out` feeds the card's own spectrum -- a kick on the bass
bins every half second, a little hiss above -- through the host's element
setter, 40 frames at 60 fps, so the Onset envelope is live on the frame
that is read back.

**Azimuth's two ends are the same light.** 0 and 360 degrees are both
north, so the ends are identical by construction; it is swept 0 against 0.5
(north against south) in CONTEXT.

**Most controls need the defaults' context**: Z Factor only shows through
Hillshade (0.6 by default), the line colour only where there are lines, the
water colour only where there is sea. The defaults have all three, on
purpose; a new control that needs something else must say so in CONTEXT.

**An option's range is its element count, an integer's is its own.** An
option parameter's range reads back 0..1 whatever its element count, so
`cntest --list` prints the count as the range; Index Every is a real integer
and prints 2..10. The ends swept are those.
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "cntest"))
    ...
    scratch = tempfile.mkdtemp(prefix="cnsweep")

    # One render first: a runner with no GL at all cannot sweep anything, and
    # that has to read as a skip, loudly, not as thirty dead controls.
