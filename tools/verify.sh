...
#                 strings `cntest --dump-shaders` writes; and does any of
...
#                   --controls  every control mapping against the README
#                   --detector  the sea's audio detector: primed, its gain,
#                               its releases measured from its output, jumps
#                   --clock     a six-day millisecond host clock drives the
#                               sea as a fresh seconds one does
...
#                 the picture:
#                   --count     contours crossed along a ramp, exactly
#                   --spacing   interval / g apart, whole-pixel and
#                               fractional, separately
#                   --width     one pen width on slopes spanning 8:1
#                   --hillshade planes of known slope and aspect shade to
#                               the cartographer's closed form
#                   --sea       water exactly below sea level; the shoreline
#                               centred on its isoline
#                   --index     every Nth contour heavy, and only those
#                   --flat      a noisy plateau ON a level draws nothing
#                   --audio     the music raises the sea; onset primed
#                   --resize    a resize mid-run keeps the drive and leaves
#                               no stale buffer
...
step "shaders"
if out=$(tools/check-shaders.sh "$CNTEST" 2>&1); then
	pass "$( printf '%s\n' "$out" | tail -1 | sed 's/^ *//' )"
else
...
dump=$( mktemp -d )
"$CNTEST" --dump-shaders "$dump" >/dev/null
...
step "offline (no GL)"
for check in controls detector clock names negative-offline; do
	if out=$("$CNTEST" --$check 2>&1); then
...
for size in 320x180 1280x720; do
	...
	for check in count spacing width hillshade sea index flat audio resize negative; do
		if out=$("$CNTEST" --$check --size $size 2>&1); then
			pass "cntest --$check: $( printf '%s\n' "$out" | grep -v '^$' | grep -v 'checks,' | tail -1 )"
		else
			fail "cntest --$check at $size"
			printf '%s\n' "$out" | sed 's/^/      /'
		fi
	done
done
...
got=$( "$CNTEST" --pipe --size 64x36 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
# pipeline whatever cntest did, and the refusal would pass for the wrong reason.
printf '0 No Such Control 0.5\n' > "$cues"
"$CNTEST" --pipe --size 64x36 --script "$cues" < "$raw" >/dev/null 2>&1
status=$?
if [ "$status" -eq 2 ]; then
...
got=$( "$CNTEST" --pipe --size 64x36 --fail-render-at 1 < "$raw" 2>/dev/null | wc -c | tr -d ' ' )
status=${PIPESTATUS[0]}
...
"$CNTEST" --pipe --size 64x36 < "$many" 2>/dev/null | head -c 100 >/dev/null
status=${PIPESTATUS[0]}
if [ "$status" -eq 1 ]; then
...
step "sweep"
if out=$(python3 tools/sweep.py --binary "$CNTEST" 2>/dev/null); then
	pass "$( printf '%s\n' "$out" | tail -1 )"
else
...
"$CNTEST" --bench --frames 60 2>&1 | sed -n '3,6p' | sed 's/^/   /'

BUNDLE="$BUILD/Contour.bundle"
BIN="$BUNDLE/Contents/MacOS/Contour"
...
if [ "$(uname)" = "Darwin" ] && [ -d "$BUNDLE" ]; then
	...
	if [ "$ident" = "com.stoatworks.ffgl.contour" ]; then
	...
	if codesign --force --sign - --timestamp=none "$tmp/Contour.bundle" >/dev/null 2>&1; then
	...
	if [ -x "$OXBOW" ]; then
		...
		for want in "name:        SW Contour" "id:          CN01" "type:        effect"; do
			case "$probe" in
